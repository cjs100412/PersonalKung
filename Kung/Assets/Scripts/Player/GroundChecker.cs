...
public class GroundChecker : MonoBehaviour
{
    ...
    public float groundNormalThreshold = 0.8f; // 땅으로 간주할 노멀 벡터의 Y값 (0.7f ~ 1.0f 사이 권장)

    private bool isGrounded; // 현재 땅에 닿아있는지 여부
    ...
    void CheckForGround(Collision2D collision)
    {
        ...
        foreach (ContactPoint2D contact in collision.contacts)
        {
            ...
            if (Vector3.Dot(normal, Vector3.up) >= groundNormalThreshold)
            {
                Debug.Log("닿음");
                isGrounded = true;
